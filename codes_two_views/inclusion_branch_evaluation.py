...
def predict_video(cabin_video_path, face_video_path, args):
    ...
    cabin_clips, face_clips = clip_generation(cabin_video_path, face_video_path, clip_length, clip_stride)
...
#     print(all_end_scores)
    ...
    return selected_starts, selected_ends, all_start_scores
...
#     all_predicted_ends_accuracy = []
    ...
    for cabin_video in selected_cabin_videos:
        ...
        selected_starts, selected_ends, all_start_scores = predict_video(cabin_video_path, face_video_path, args)
        ...
        true_negative = len(all_start_scores)-len(GT_starts)-false_positive
        false_negative = len(GT_starts) - true_positive
        ...
        recall = float(true_positive)/(len(GT_starts) + 1e-10)
        ...
        true_negative1 = len(all_start_scores)-len(GT_ends)-false_positive1
        false_negative1 = len(GT_ends) - true_positive1
        ...
        recall1 = float(true_positive1)/(len(GT_ends) + 1e-10)
...
#         [start_frame_list, end_frame_list] = key_frame_dict[cabin_video]
#         print(cabin_video)
#         print(start_frame_list, end_frame_list)
#         print(selected_starts, selected_ends)
#         total_starts += len(start_frame_list)
#         total_ends += len(end_frame_list)
#         total_predicted_starts += len(selected_starts)
#         total_predicted_ends += len(selected_ends)
#         if selected_starts != []:
#             for selected_start in selected_starts:
#                 for start_frame in start_frame_list:
#                     if start_frame in list(range(selected_start*4+1, selected_start*4+17)):
#                         correct_predicted_starts += 1

#         if selected_ends != []:
#             for selected_end in selected_ends:
#                 for end_frame in end_frame_list:
#                     if end_frame in list(range(selected_end*4+1, selected_end*4+17)):
#                         correct_predicted_ends += 1
                   
#     start_recall = float(correct_predicted_starts)/total_starts
#     start_precision = float(correct_predicted_starts)/total_predicted_starts
#     end_recall = float(correct_predicted_ends)/total_ends
#     end_precision = float(correct_predicted_ends)/total_predicted_ends
#     print('start_recall:{}'.format(start_recall), 'start_precision:{}'.format(start_precision), 'end_recall:{}'.format(end_recall), 'end_precision:{}'.format(end_precision))
